-- Auto-cleanup of stale waiting queue entries.
-- A waiting entry older than 5 minutes is an orphan (user closed browser,
-- network dropped, or app crashed before the 30-second timeout could cancel).
-- This function is called at the start of each rpc_enter_matchmaking so cleanup
-- happens naturally without requiring a separate cron job.
...
-- Allow authenticated users to trigger cleanup (called within rpc_enter_matchmaking context)
...
-- Also update rpc_enter_matchmaking to call cleanup at entry time.
-- This replaces the previous version and adds the cleanup call before ban check.
...
BEGIN
  ...
  -- Clean up orphaned waiting entries from other users first (best-effort).
  ...
  -- Cancel stale waiting entries from previous attempts by this user.
  UPDATE public.match_queue
     SET status = 'cancelled'
  ...
  -- Weekly free-entry reset runs server-side.
  ...
  -- Subscribers get priority: matched before free users when multiple are waiting.
