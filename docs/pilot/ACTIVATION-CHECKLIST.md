...
All 40 invite entries in [wave1-invites.template.csv](templates/wave1-invites.template.csv) use synthetic placeholder addresses (`pilot.canberraXX@verity.date`). These addresses:
...
Create a private runtime copy and replace placeholder emails with real addresses:

```bash
mkdir -p private/pilot
cp docs/pilot/templates/wave1-invites.template.csv private/pilot/wave1-invites.csv
```
